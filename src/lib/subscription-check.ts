...
export async function checkSubscription(userId: string): Promise<SubscriptionStatus> {
  ...
  if (!isOnline) {
    return {
      ...
      message: "Você precisa de internet para usar o BCRM. Conecte-se à internet e tente novamente.",
    };
  }
  ...
  try {
    ...
    return {
      ...
      message: isActive ? undefined : "Sua assinatura está expirada. Renove para continuar usando o BCRM.",
    };
  } catch {
    return {
      isOnline: true,
