...
export class CapacitorDatabaseAdapter {
  ...
  async createUser(data: any) {
    ...
    await database.run(`INSERT INTO users (id, name, email, password_hash, role) VALUES (?, ?, ?, ?, ?)`, [
      id,
      data.name,
      data.email,
      data.passwordHash,
  ...
  async getCustomers() {
    const database = getCapacitorDB();
    const result = await database.query("SELECT * FROM customers WHERE deleted_at IS NULL ORDER BY created_at DESC");
  ...
  async deleteCustomer(id: string) {
    const database = getCapacitorDB();
    await database.run("UPDATE customers SET deleted_at = datetime('now'), is_dirty = 1 WHERE id = ?", [id]);
  }

  async getDirtyRecords(table: string, userId: string) {
    const database = getCapacitorDB();
    const result = await database.query(`SELECT * FROM ${table} WHERE user_id = ? AND is_dirty = 1`, [userId]);
    return result.values ?? [];
  }
